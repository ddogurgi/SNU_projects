...
template <typename T>
void check_parent_child_link(FibonacciNode<T>* n) {
    if(!n->child)
        return;
    ...
    for(FibonacciNode<T>* child: children_list) {
        REQUIRE(node->parent.lock().get() == n);
        check_parent_child_link(child);
    }
}
...
TEST_CASE("Random Insertion and Extraction", "[Integer]") {
    ...
    for(size_t i = 0 ; i < N; ++i) {
        ...
        for(FibonacciNode<int>* root : root_list) {
            ...
            check_min_heap_property(root);
            check_parent_child_link(root);
        }
    }

}

TEST_CASE("Other error handlings", "[Integer]") {
    SECTION("Extraction from an empty heap") {
        FibonacciHeap<int> heap = {};
        REQUIRE(heap.extract_min() == std::nullopt);
        REQUIRE(heap.size() == 0);
        REQUIRE(heap.is_empty() == true);
    }
}