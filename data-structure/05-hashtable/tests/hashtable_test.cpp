...
#define LinearStrStrHt LinearProbeHashTable<std::string, std::string>
#define QuadStrStrHt QuadProbeHashTable<std::string, std::string>
...
template <typename HT>
void string_key_test(HT &htable) {
    REQUIRE(htable.put("aa", "1") > 0);
    REQUIRE(htable.put("ab", "2") > 0);
    REQUIRE(htable.put("ab", "3") == -1);

    std::string value;
    REQUIRE(htable.get("aa", value) > 0);
    REQUIRE(value == "1");

    REQUIRE(htable.get("ab", value) > 0);
    REQUIRE(value == "2");
}

TEST_CASE("hashtable string key test", "[hashtable]") {
    HashFunc *hash_func = new DefaultHashFunc;
    
    LinearStrStrHt linear_ht(hash_func);
    string_key_test<LinearStrStrHt>(linear_ht);

    QuadStrStrHt quad_ht(hash_func);
    string_key_test<QuadStrStrHt>(quad_ht);
