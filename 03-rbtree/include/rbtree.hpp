...
RBNode<T>* RBNode<T>::insert(std::unique_ptr<RBNode<T>>& n, const T& t) {
    // TODO
    if(n == nullptr) return new RBNode<T>(t);
    if(n->key == t) ;
    else if(n->key > t) n->left.reset(insert(n->left, t));
    else n->right.reset(insert(n->right, t));
    if(is_red(n->right) && !is_red(n->left)) n.reset(rotate_left(n));
    if(is_red(n->left) && is_red(n->left->left)) n.reset(rotate_right(n));
    if(is_red(n->left) && is_red(n->right)) n->flip_color();
...
RBNode<T>* RBNode<T>::remove(std::unique_ptr<RBNode<T>>& n, const T& t) {
    ...
    if(t < n->key){
        ...
        if(is_red(n->left)) n.reset(rotate_right(n));
        if(t == n->key && n->right == nullptr) return nullptr;
        if(!is_red(n->right) && !is_red(n->right->left)) n.reset(move_red_right(n));
        if(t == n->key){
            n->key = n->right->leftmost_key();
            n->right.reset(remove_min(n->right));
        }
        else n->right.reset(remove(n->right, t));
    }
    return fix_up(n);
}
